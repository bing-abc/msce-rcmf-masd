...
from tg_prediction_pipeline.models import RCMFConfig, RCMFRegressor
...
from tg_prediction_pipeline.models import MSCERegressor


def default_rcmf_output_dir() -> Path:
    return Path(__file__).resolve().parents[3] / "artifacts" / "rcmf_benchmark"


def _default_rcmf_config_path() -> Path:
    return Path(__file__).resolve().parents[3] / "configs" / "rcmf.yaml"


def load_rcmf_config(config_path: str | Path | None = None) -> RCMFConfig:
    path = Path(config_path) if config_path is not None else _default_rcmf_config_path()
    payload = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
    stage_payload = payload.get("rcmf_stage", {})
    loss_payload = stage_payload.get("loss", {})
    return RCMFConfig(
        hidden_dim=int(stage_payload.get("hidden_dim", 96)),
        ...
        delta_bound_k=float(stage_payload.get("delta_bound_k", 40.0)),
        blend_grid_size=int(stage_payload.get("blend_grid_size", 21)),
        residual_alpha=float(stage_payload.get("residual_alpha", 3.0)),
        trust_margin_weight=float(loss_payload.get("trust_margin_weight", 0.15)),
        delta_weight=float(loss_payload.get("delta_weight", 0.01)),
    )


def _select_rows(matrix: np.ndarray, indices: tuple[int, ...]) -> np.ndarray:
    return np.asarray(matrix, dtype=np.float32)[list(indices)]


def _select_targets(targets: np.ndarray, indices: tuple[int, ...]) -> np.ndarray:
...
def _anchor_spec(suite: Any, anchor_name: str) -> Any:
    for spec in suite.baseline_specs:
        if spec.name == anchor_name:
            return spec
    raise ValueError(f"anchor baseline '{anchor_name}' is not defined")
...
def _fit_anchor(
    *,
    ...
    model.fit(_select_rows(descriptor_matrix, split.train_indices), _select_targets(targets, split.train_indices))
    return {
        "train": np.asarray(model.predict(_select_rows(descriptor_matrix, split.train_indices)), dtype=np.float32).reshape(-1),
        "val": np.asarray(model.predict(_select_rows(descriptor_matrix, split.val_indices)), dtype=np.float32).reshape(-1),
        "test": np.asarray(model.predict(_select_rows(descriptor_matrix, split.test_indices)), dtype=np.float32).reshape(-1),
        "external": np.asarray(model.predict(_select_rows(descriptor_matrix, split.external_indices)), dtype=np.float32).reshape(-1),
    }
...
def _result_row(
    *,
    ...
    delta_vs_previous_test_mae: float,
    delta_vs_previous_test_hard_mae: float,
    delta_vs_previous_external_mae: float,
    delta_vs_previous_external_hard_mae: float,
    ...
    row: dict[str, Any] = {
        ...
        "delta_vs_previous_test_mae": float(delta_vs_previous_test_mae),
        "delta_vs_previous_test_hard_mae": float(delta_vs_previous_test_hard_mae),
        "delta_vs_previous_external_mae": float(delta_vs_previous_external_mae),
        "delta_vs_previous_external_hard_mae": float(delta_vs_previous_external_hard_mae),
        "improvement_vs_previous_test_mae": float(-delta_vs_previous_test_mae),
        "improvement_vs_previous_test_hard_mae": float(-delta_vs_previous_test_hard_mae),
        "improvement_vs_previous_external_mae": float(-delta_vs_previous_external_mae),
        "improvement_vs_previous_external_hard_mae": float(-delta_vs_previous_external_hard_mae),
    }
...
def _stage_diagnostics(details: dict[str, np.ndarray], prefix: str) -> dict[str, Any]:
    return {
        f"{prefix}_backend_name": str(np.asarray(details["backend_name"], dtype=object).reshape(-1)[0]),
        f"{prefix}_blend_alpha_mean": float(np.asarray(details["blend_alpha"], dtype=np.float64).mean()),
        f"{prefix}_abs_delta_mean": float(np.abs(np.asarray(details["bounded_delta"], dtype=np.float64)).mean()),
        f"{prefix}_raw_abs_delta_mean": float(np.abs(np.asarray(details["raw_bounded_delta"], dtype=np.float64)).mean()),
    }


def _write_summary(results_frame: pd.DataFrame, output_dir: Path) -> pd.DataFrame:
    results_frame.to_csv(output_dir / "rcmf_results.csv", index=False)
    metric_columns = [
        "test_mae",
        "test_rmse",
        "test_hard_mae",
        ...
        "delta_vs_previous_test_mae",
        "delta_vs_previous_test_hard_mae",
        "delta_vs_previous_external_mae",
        "delta_vs_previous_external_hard_mae",
        "improvement_vs_previous_test_mae",
        "improvement_vs_previous_test_hard_mae",
        "improvement_vs_previous_external_mae",
        "improvement_vs_previous_external_hard_mae",
    ]
    ...
    summary_frame.to_csv(output_dir / "rcmf_summary.csv", index=False)
    return summary_frame


def run_rcmf_benchmark(
    *,
    ...
    output_root = Path(output_dir) if output_dir is not None else default_rcmf_output_dir()
    ...
    for split in splits:
        ...
        rcmf_train_seed = int(split.random_state * 101 + 71)
        ...
        msce_train_details = msce_regressor.predict_details(
            _select_rows(descriptor_matrix, split.train_indices),
            _select_rows(context_matrix, split.train_indices),
            anchor_predictions["train"],
        )
        msce_val_details = msce_regressor.predict_details(
            _select_rows(descriptor_matrix, split.val_indices),
            _select_rows(context_matrix, split.val_indices),
            anchor_predictions["val"],
        )
        msce_test_details = msce_regressor.predict_details(
            _select_rows(descriptor_matrix, split.test_indices),
            _select_rows(context_matrix, split.test_indices),
            anchor_predictions["test"],
        )
        msce_external_details = msce_regressor.predict_details(
            _select_rows(descriptor_matrix, split.external_indices),
            _select_rows(context_matrix, split.external_indices),
            anchor_predictions["external"],
        )

        rcmf_regressor = RCMFRegressor(rcmf_config, train_seed=rcmf_train_seed)
        rcmf_regressor.fit(
            descriptor_train=_select_rows(descriptor_matrix, split.train_indices),
            context_train=_select_rows(context_matrix, split.train_indices),
    ...
    manifest = {
        ...
        "msce_config": dict(load_msce_config(msce_config_path).__dict__),
        "rcmf_config": dict(rcmf_config.__dict__),
        "dataset_summary": summarize_dataset(dataset).to_dict(orient="records"),
        "artifacts": {
            "results_csv": "rcmf_results.csv",
            "summary_csv": "rcmf_summary.csv",
            "splits_json": "protocol_splits.json",
            "manifest_json": "rcmf_manifest.json",
        },
    }
    (output_root / "rcmf_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    return results_frame, summary_frame


def main() -> int:
    parser = ArgumentParser(description="Run the standalone RCMF stage benchmark.")
